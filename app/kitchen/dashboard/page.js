...
export default function Dashboard() {
  ...
  const navs = [
    {
      ...
      link: "/kitchen/dashboard/register",
    },
    {
      title: "ثبت مواد",
      link: "/kitchen/dashboard/form",
    },
    {
      title: "صفحه اصلی",
      link: "/",
  ...
  useEffect(() => {
    ...
    if (role !== "admin") router.push("/kitchen/dashboard/form")
    ...
    <main className="">
      <section className="relative w-full h-90  z-0">
        <div className="absolute w-full h-90  z-10">
          <img src="../../images/image-2.jpeg" className="h-90 w-full" />
        </div>
        ...
        <nav className=" absolute w-full text-xl p-4 flex items-center justify-between z-100000">
          {/* <div className="flex gap-12">
            ...
            <button
              onClick={enterPanel}
              className=" ms-4  bg-green-500 text-gray-900 h-10 p-1 px-4 rounded hover:shadow shadow-amber-900 transition-all duration-500">
              پنل ورودی
            </button>
            <button
              onClick={exportPanel}
            ...
            {currentItems.map((item) => (
              <tr key={item.id} className="border-t text-center text-gray-700">
                ...
                <td className="flex gap-2 justify-center items-center">
                  ...
                  <Link
                    href={`/kitchen/dashboard/edit/${item.id}`}
