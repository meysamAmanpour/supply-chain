...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    document.cookie = `authOut=true; path=/`
    document.cookie = `role=${data.user.role}; path=/`
    document.cookie = `username=${data.user.username}; path=/`

    // role-based redirect
    if (data.user.role === "admin") router.push("/export/dashboard")
    else router.push("/export/dashboard/form")
  }
  ...
  const handleRegister = async (e) => {
    ...
    const res = await fetch("/api/export/register", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
