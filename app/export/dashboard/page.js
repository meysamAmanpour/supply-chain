...
export default function Dashboard() {
  ...
  const navs = [
    {
      ...
      link: "/export/dashboard/register",
    },
    {
      title: "ثبت مواد",
      link: "/export/dashboard/form",
    },
    {
      title: "صفحه اصلی",
      link: "/",
  ...
  useEffect(() => {
    ...
    if (!role) router.push("/export/login")
    if (role !== "admin") router.push("/export/dashboard/form")
  ...
  const fetchMaterials = async () => {
    try {
      const res = await fetch("/api/export/materials")
  ...
  const handleLogout = async () => {
    ...
    await fetch("/api/export/logout", { method: "POST" })
    window.location.href = "/export/login"
  }

  useEffect(() => {
    const handler = setTimeout(() => {
      setDebounceSearch(supplierFilter)
    }, 1000) //1000ms تاخیر
  ...
  const handleDelete = async (id) => {
    ...
    if (res.ok) {
      ...
      const newFiltered = newMaterials.filter(
        (item) =>
          (supplierFilter === "" || item.supplier.includes(debounceSearch)) &&
          (fromDate !== "" ||
            toDate === "" ||
  ...
  const downloadPDF = () => {
    ...
    const to = toDate || "_"
    const receiver = supplierFilter || "همه"

    doc.text(`از تاریخ: ${from}`, 190, 25, { align: "right" })
    doc.text(`تا تاریخ: ${to}`, 190, 32, { align: "right" })

    doc.text(`تحویل گیرنده:  ${supplier}`, 190, 39, { align: "right" })
  ...
  const downloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "گزارش مواد خروجی")
  ...
  return (
    <main className="">
      <section className="relative w-full h-90  z-0">
        <div className="absolute w-full h-90 bg-red-400 z-10">
          <img src="../../images/headerImage.jpg" className="h-90 w-full" />
        </div>
        ...
        <nav className=" absolute w-full text-xl p-4 flex items-center justify-between z-100000">
          {/* <div className="flex gap-12">
            ...
            <button
              ...
              className=" ms-4  bg-blue-500 text-gray-900 h-10 p-1 px-4 rounded hover:shadow shadow-green-950 transition-all duration-500">
              پنل مصرفی
            </button>
          </div> */}
        ...
        {report.length > 0 && (
          <div className=" mt-4 flex flex-col items-center  justify-center">
            ...
            <table className="w-100 border ">
              ...
              <tbody>
                {report.map((r, index) => (
                  <tr key={index} className="bg-amber-300">
            ...
            {currentItems.map((item) => (
              <tr key={item.id} className="border-t text-center text-gray-700">
                ...
                <td className="flex gap-2 justify-center items-center">
                  ...
                  <Link
                    href={`/export/dashboard/edit/${item.id}`}
